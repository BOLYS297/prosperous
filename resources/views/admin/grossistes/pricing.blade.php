                    ...
                    @foreach ($produits as $produit)
                        ...
                        <tr data-search="{{ \Illuminate\Support\Str::lower(trim($produit->nom . ' ' . $produit->reference)) }}">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <input type="number" name="prix[{{ $loop->index }}][prix_achat]" step="0.01" min="0" class="w-32 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" value="{{ $prixAchat }}">
                            </td>
                            <td class="px-6 py-4">
                                <input type="number" name="prix[{{ $loop->index }}][prix_vente]" step="0.01" min="0" class="w-32 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600" value="{{ $prixVente }}" required>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
